...
(function () {
    ...
    let K = {
        ...
        injectJS: function (text, sURL) {
            let tgt, scriptNode = document.createElement('script');
    ...
    function makeTextSprite(text, params) {
        ...
        let mesh = new THREE.Mesh(new THREE.PlaneGeometry(canvas.width, canvas.height), new THREE.MeshBasicMaterial({
            map: texture,
    ...
    function captureImage() {
        ...
        if (twoDCanvas && tomni.gameMode) {
            let twoDParent = $('#twoD').parent()[0], minX = Math.floor((threeDCanvas.width - twoDParent.clientWidth) / 2), maxX = Math.floor(threeDCanvas.width / 2);
            scrCtx.drawImage(threeDCanvas, minX, 0, maxX, threeDCanvas.height, 0, 0, maxX, threeDCanvas.height);
            let g = parseTransform($(twoDCanvas).css('transform')), m = parseFloat($(twoDCanvas).css('left')), p = -1 * parseFloat($(twoDCanvas).css('bottom')), h = {
                x: 0,
                y: 0
            }, threeDCanvasDims = {
                x: maxX,
